...
/** An [[ExecutionScheme]] that is capable of using [[sangria.catseffect.schema.AsyncValue]].
  ...
  */
class AsyncExecutionScheme[F[_]: Async](val futureToF: FutureToF[F]) extends ExecutionScheme {
  ...
  override val resolverBuilder: ResolverBuilder = new AsyncResolverBuilder[F](futureToF)
