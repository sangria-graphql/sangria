...
private[execution] class AsyncResolverBuilder[F[_]: Async](futureToF: FutureToF[F])
...
private[execution] class AsyncResolver[Ctx, F[_]: Async](
  ...
  private def liftFuture[A](future: => Future[A]): F[A] = futureToF.liftFuture(future)
  ...
  private def resolveStandardFieldResolutionSeqInnerF(
    ...
    def resolveAsyncLike(lifted: F[Any]): F[Result] =
      asyncF.attempt(lifted).flatMap {
        case Left(e) =>
          asyncF.pure(
            Result(
    ...
    def resumeSeqDefer(outcomes: Vector[(DeferOutcome, Long)]): F[Resolve] = outcomes match {
      case Vector((Left(e), _)) =>
        asyncF.pure(Result(ErrorRegistry(fieldPath, resolveErrorFn(e), fields.head.location), None))
      case Vector((Right((v, es)), newLineage)) =>
        val updatedUc = resolveUcFn(v)
        es.foreach(resolveErrorFn)
        resolveValueTypeF(fieldPath, fields, sfield.fieldType, sfield, resolveValFn(v), updatedUc)
    ...
    result match {
      ...
      case DeferredValue(d) =>
        ...
        runToCompletion(uc, Pending(Vector(defer), Vector(NoLineage), resumeSeqDefer))

      case FutureValue(f) => resolveAsyncLike(liftFuture(f))

      case a: AsyncValue[Ctx, Any, F] => resolveAsyncLike(a.value)
