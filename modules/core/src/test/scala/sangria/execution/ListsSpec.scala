...
class ListsSpec extends AnyWordSpec with Matchers with FutureResultSupport {
  ...
  def check(testType: OutputType[Any], testData: Action[Unit, Any], expected: Any) = {
    ...
    lazy val Type: ObjectType[Unit, Data] = ObjectType(
      "DataType",
      () =>
        fields[Unit, Data](
    ...
    val doc = QueryParser.parse("{ nest { test } }").success.value
    ...
    "[T]" when {
      ...
      "Future[List[T]]" should {
        ...
        "Contains null" in check(
          ...
          Map("data" -> Map("nest" -> Map("test" -> List(1, null, 2)))))
        "Returns None" in check(
          tpe,
          success(None),
          Map("data" -> Map("nest" -> Map("test" -> null))))
        "Returns null" in check(
          tpe,
          FutureValue(success(null)),
          Map("data" -> Map("nest" -> Map("test" -> null))))
        "Rejected" in check(
          tpe,
