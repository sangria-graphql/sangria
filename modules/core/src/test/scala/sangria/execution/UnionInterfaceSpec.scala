...
class UnionInterfaceSpec
  ...
  val CatType = ObjectType(
    ...
    fields[Unit, Cat](Field("meows", OptionType(BooleanType), resolve = _.value.meows)))

  val PetType = UnionType[Unit]("Pet", types = DogType :: CatType :: Nil)

  val Pet2Type = UnionType[Unit]("Pet2", types = DogType :: CatType :: Nil)
    .mapValue[Either[Dog, Cat]](_.fold(dog => dog: Any, cat => cat: Any))
  ...
  val bob =
    Person(Some("Bob"), Some(List(Some(garfield), Some(odie))), Some(List(Some(liz), Some(odie))))
  ...
  "Execute: Union and intersection types" should {
    ...
    "caching should respect output object polymorphism" in {
      ...
      val QuzType = ObjectType(
        "Quz",
        fields[Unit, Quz](
          Field("id", StringType, resolve = _.value.id),
          Field("i", IntType, resolve = _.value.i)))

      val BazType = ObjectType(
        "Baz",
        fields[Unit, Baz](
          Field(
            "quz",
      ...
      val FooBarType = InterfaceType(
        "FooBar",
        fields[Unit, FooBar](Field("baz", OptionType(BazType), resolve = _.value.baz)))
      ...
      val QueryType = ObjectType(
        "Query",
        fields[Unit, List[Any]](
          Field(
            "foo",
            OptionType(ListType(OptionType(FooBarBazType))),
            resolve = _.value.map(Some(_)))))
      ...
      Executor.execute(schema, query, root = data).await should be(
        Map("data" -> Map("foo" -> Vector(
          Map("__typename" -> "Foo", "baz" -> Map("quz" -> Vector(Map("id" -> "one")))),
          Map("__typename" -> "Baz"),
          Map("__typename" -> "Bar", "baz" -> Map("quz" -> Vector(Map("id" -> "two", "i" -> 2))))
        ))))
    }
  }
}
