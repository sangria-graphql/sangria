...
class ExecutorSchemaSpec extends AnyWordSpec with Matchers with FutureResultSupport {
  ...
  case class Author(id: Option[String], name: Option[String], recentArticle: Option[String]) {
    def pic(width: Int, height: Int): Option[Image] = Some(
      Image(Some("cdn://123"), Some(width), Some(height)))
  }
  ...
  val BlogArticleType: ObjectType[Unit, Article] = ObjectType(
    "Article",
    fields[Unit, Article](
  ...
  val BlogQueryType = ObjectType(
    ...
    fields[Unit, Unit](
      ...
      Field(
        "feed",
        OptionType(ListType(OptionType(BlogArticleType))),
        resolve = _ => (1 to 10).toList.map(article))
    )
  )

  val BlogSubscriptionType = ObjectType(
    "Subscription",
    fields[Unit, Unit](
      Field(
        "articleSubscribe",
  ...
  def article(id: Int): Option[Article] = Some(
    Article(
      id = s"$id",
