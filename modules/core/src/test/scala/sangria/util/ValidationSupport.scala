...
trait ValidationSupport extends Matchers  {
  ...
  val schema = Schema(QueryRoot, directives = BuiltinDirectives ++ List(
    ...
    Directive("genericDirectiveA", locations = Set(DirectiveLocation.FragmentDefinition, DirectiveLocation.Field), shouldInclude = alwaysInclude),
    Directive("genericDirectiveB", locations = Set(DirectiveLocation.FragmentDefinition, DirectiveLocation.Field), shouldInclude = alwaysInclude),
