...
object CatsScenarioExecutor extends FutureResultSupport {
  ...
  val ResolvePromiseString = Directive(
    "resolvePromiseString",
    arguments = ValueArg :: Nil,
    locations = Set(DirectiveLocation.FieldDefinition))
  val ResolveEmptyObject =
    Directive("resolveEmptyObject", locations = Set(DirectiveLocation.FieldDefinition))
  val ResolveTestData = Directive(
    "resolveTestData",
    arguments = NameArg :: Nil,
    locations = Set(DirectiveLocation.FieldDefinition))
  val ResolvePromiseTestData = Directive(
    "resolvePromiseTestData",
    arguments = NameArg :: Nil,
    locations = Set(DirectiveLocation.FieldDefinition))
  val ResolvePromise =
    Directive("resolvePromise", locations = Set(DirectiveLocation.FieldDefinition))
  val ResolveError = Directive(
    "resolveError",
    arguments = MessageArg :: Nil,
    locations = Set(DirectiveLocation.FieldDefinition))
  val ResolveErrorList = Directive(
    "resolveErrorList",
    arguments = ValuesArg :: MessagesArg :: Nil,
    locations = Set(DirectiveLocation.FieldDefinition))
  val ResolvePromiseReject = Directive(
    "resolvePromiseReject",
    arguments = MessageArg :: Nil,
    locations = Set(DirectiveLocation.FieldDefinition))
  val resolvePromiseRejectList = Directive(
  ...
  def schemaBuilder(testData: JsValue): AstSchemaBuilder[Any] = AstSchemaBuilder.resolverBased[Any](
    DirectiveResolver(
      ResolveString,
      c => correctValue(c.ctx.field.fieldType, replacePlaceholders(c.arg(ValueArg), c.ctx.args))),
    DirectiveResolver(
      ArgumentsJson,
      c => {
    ...
    DirectiveResolver(
      ResolveTestData,
      c => correctValue(c.ctx.field.fieldType, testData(c.arg(NameArg)))),
    DirectiveResolver(
      ResolvePromiseTestData,
      c =>
        Future {
    ...
    AnyFieldResolver { case _ =>
      c =>
        extractCorrectValue(
          c.field.fieldType,
          c.value.asInstanceOf[JsValue].get(c.field.name),
          testData)
    },
    InstanceCheck(c =>
      (value, _) =>
  ...
  def extractCorrectValue(tpe: OutputType[_], value: Option[JsValue], testData: JsValue): Any =
    tpe match {
      ...
      case ListType(ofType) =>
        value.get.arrayValue.map(v => extractCorrectValue(ofType, Option(v), testData))
  ...
  def replacePlaceholders(template: String, args: Args) =
    args.raw.keys.foldLeft(template) { case (acc, key) =>
      acc.replaceAll(
        "\\$" + key,
        args.arg[Any](key) match {
          case Some(v) => "" + v
          case None => ""
          case v => "" + v
        })
...
object CatsAssertions extends Matchers {
  ...
  def sameLocations(violation: Violation, locations: List[ErrorLocation]) =
    if (!violation.isInstanceOf[AstNodeLocation] && locations.nonEmpty) false
      ...
      withLoc.locations.size == locations.size && withLoc.locations.zipWithIndex.forall {
        case (pos, idx) =>
...
object CatsScenarioData {
  ...
  case class ErrorsContain(message: Either[String, Pattern], locations: List[ErrorLocation])
      extends Assertion
  case class ErrorCode(code: String, args: Map[String, String], locations: List[ErrorLocation])
      extends Assertion
  ...
  def getTestData(value: Option[YamlValue], path: String) =
    value
      ...
      .orElse(
        value
          .flatMap(_.get("test-data-file"))
          .map(f =>
            FileUtil.loadTestData(path + "/" + f.stringValue) match {
  ...
  def getAction(value: YamlValue, testName: String, testData: JsValue): Action = {
    ...
    when
      .get("validate")
      .map(v =>
        Validate(v.arrayValue.toList.map(name =>
          QueryValidator.allRules
            .find(_.getClass.getSimpleName == name.stringValue)
  ...
  def getErrorArgs(value: YamlValue) = value.get("args") match {
    case Some(YamlObject(elems)) =>
      elems.map { case (key, value) => key.stringValue -> value.stringValue }
  ...
  def getAssertion(value: YamlValue, testName: String): Assertion =
    value
      ...
      .orElse(value
        .get("error")
        .map(v => ErrorsContain(Left(v.stringValue), getErrorLocations(value).toList)))
      .orElse(value
        .get("error-regex")
        .map(v => ErrorsContain(Right(v.stringValue.r.pattern), getErrorLocations(value).toList)))
      .orElse(value
        .get("error-code")
        .map(v => ErrorCode(v.stringValue, getErrorArgs(value), getErrorLocations(value).toList)))
      .orElse(value.get("exception").map(v => ExceptionContain(Left(v.stringValue))))
      .orElse(
        value.get("exception-regex").map(v => ExceptionContain(Right(v.stringValue.r.pattern))))
...
object JsonAndYamlHelpers {
  implicit class YamlOps(value: YamlValue) {
    ...
    def intValue = value.asInstanceOf[YamlNumber].value match {
      ...
      case v =>
        throw new IllegalArgumentException(s"Unsupported Int '$v' of class '${v.getClass}'.")
    }
  }
  ...
  def convertToJson(value: YamlValue): JsValue = value match {
    ...
    case YamlObject(fields) =>
      JsObject(fields.map { case (k, v) => k.stringValue -> convertToJson(v) })
