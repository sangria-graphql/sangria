...
class OverlappingFieldsCanBeMerged extends ValidationRule {
  override def visitor(ctx: ValidationContext) = new AstValidatingVisitor {
    ...
    val cachedFieldsAndFragmentNames = new MutableMap[
      (Set[String], Vector[ast.Selection]),
      (MutableMap[String, ListBuffer[AstAndDef]], LinkedHashSet[String])]()
    ...
    override val onEnter: ValidationVisit = {
      case selCont: ast.SelectionContainer if selCont.selections.nonEmpty =>
        ...
        if (conflicts.nonEmpty)
          Left(
            conflicts.toVector.map(c =>
              FieldsConflictViolation(
                c.reason.fieldName,
                c.reason.reason,
                ctx.sourceMapper,
                (c.fields1 ++ c.fields2).flatMap(_.location))))
        else
    ...
    def findConflictsWithinSelectionSet(
        ...
        visitedFragments: Set[String]): ListBuffer[Conflict] = {
      val conflicts = ListBuffer[Conflict]()

      val (fieldMap, fragmentNames) = getFieldsAndFragmentNames(
        parentType.asInstanceOf[Option[CompositeType[_]]],
        selCont,
        visitedFragments)
      ...
      fragmentNames.zipWithIndex.foreach { case (fragmentName, idx) =>
        ...
        for (i <- (idx + 1) until fragmentNamesList.size)
          collectConflictsBetweenFragments(
            ...
            fragmentNamesList(i),
            visitedFragments + fragmentName,
            visitedFragments + fragmentNamesList(i),
            false)
      }
      ...
      fieldMap1.keys.foreach { outputName =>
        fieldMap2.get(outputName).foreach { fields2 =>
          ...
          for {
            ...
            visitedFragments1,
            visitedFragments2,
            parentFieldsAreMutuallyExclusive).foreach(conflicts += _)
        }
      }
    ...
    def getReferencedFieldsAndFragmentNames(
      ...
      cachedFieldsAndFragmentNames.get(visitedFragments -> fragment.selections) match {
        case Some(cached) => cached
        case None =>
          val fragmentType = ctx.schema
            .getOutputType(fragment.typeCondition, true)
            .asInstanceOf[Option[CompositeType[_]]]
    ...
    def collectFieldsAndFragmentNames(
      ...
      selCont.selections.foreach {
        ...
        case fragment: ast.InlineFragment =>
          val inlineFragmentType =
            fragment.typeCondition.flatMap(ctx.schema.getOutputType(_, true)).orElse(parentType)
    ...
    def findConflict(
      ...
      val areMutuallyExclusive =
        parentFieldsAreMutuallyExclusive || ((parentType1, parentType2) match {
          case (Some(pt1: ObjectType[_, _]), Some(pt2: ObjectType[_, _])) if pt1.name != pt2.name =>
            true
          case _ => false
        })
      ...
      if (!areMutuallyExclusive && ast1.name != ast2.name)
        Some(
          Conflict(
            ConflictReason(
              outputName,
              Left(s"'${ast1.name}' and '${ast2.name}' are different fields")),
            ast1 :: Nil,
            ast2 :: Nil))
      else if (!areMutuallyExclusive && !sameArguments(ast1.arguments, ast2.arguments))
        Some(
          Conflict(
            ConflictReason(outputName, Left("they have differing arguments")),
            ast1 :: Nil,
            ast2 :: Nil))
      else {
        val typeRes = for {
          ...
          if (doTypesConflict(field1.fieldType, field2.fieldType)) {
            ...
            Some(
              Conflict(
                ConflictReason(
                  outputName,
                  Left(s"they return conflicting types '$type1' and '$type2'")),
                ast1 :: Nil,
                ast2 :: Nil))
          } else None
    ...
    def findConflictsBetweenSubSelectionSets(
      ...
      val (fieldMap1, fragmentNames1) =
        getFieldsAndFragmentNames(parentType1, selCont1, visitedFragments1)
      val (fieldMap2, fragmentNames2) =
        getFieldsAndFragmentNames(parentType2, selCont2, visitedFragments2)
      ...
      fragmentNames2.foreach(fragmentName =>
        collectConflictsBetweenFieldsAndFragment(
          ...
          fragmentName,
          areMutuallyExclusive,
          visitedFragments2 + fragmentName))
      ...
      fragmentNames1.foreach(fragmentName =>
        collectConflictsBetweenFieldsAndFragment(
          ...
          fragmentName,
          areMutuallyExclusive,
          visitedFragments1 + fragmentName))
      ...
      for {
        ...
        fragmentName1,
        fragmentName2,
        visitedFragments1 + fragmentName1,
        visitedFragments2 + fragmentName2,
        areMutuallyExclusive)
    ...
    def collectConflictsBetweenFragments(
      ...
      (ctx.doc.fragments.get(fragmentName1), ctx.doc.fragments.get(fragmentName2)) match {
        ...
        case (Some(f1), Some(f2))
            if comparedFragments.contains(f1.name, f2.name, areMutuallyExclusive) =>
        ...
        case (Some(f1), Some(f2)) =>
          ...
          val (fieldMap1, fragmentNames1) =
            getReferencedFieldsAndFragmentNames(f1, visitedFragments1)
          val (fieldMap2, fragmentNames2) =
            getReferencedFieldsAndFragmentNames(f2, visitedFragments2)
    ...
    def collectConflictsBetweenFieldsAndFragment(
      ...
      ctx.doc.fragments.get(fragmentName).foreach { fragment =>
        val (fieldMap2, fragmentNames2) =
          getReferencedFieldsAndFragmentNames(fragment, visitedFragments)
    ...
    def subfieldConflicts(
        conflicts: Seq[Conflict],
        outputName: String,
        ast1: ast.Field,
        ast2: ast.Field): Option[Conflict] =
      if (conflicts.nonEmpty)
        Some(
          Conflict(
            ConflictReason(outputName, Right(conflicts.map(_.reason) toVector)),
    ...
    def doTypesConflict(type1: OutputType[_], type2: OutputType[_]): Boolean =
      (type1, type2) match {
    ...
    def sameValue(v1: ast.Value, v2: ast.Value) =
      QueryRenderer.render(v1, QueryRenderer.Compact) == QueryRenderer.render(
        v2,
        QueryRenderer.Compact)
