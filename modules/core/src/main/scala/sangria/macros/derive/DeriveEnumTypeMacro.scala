...
class DeriveEnumTypeMacro(context: blackbox.Context) extends {
  ...
  private def collectEnumValues(
    ...
    if (extractedValues.isEmpty)
      ...
      extractedValues.map { value =>
        ...
        val configDescr = config.collect { case MacroDocumentValue(`name`, tree, _, _) =>
          tree
        }.lastOption
  ...
  private def extractEnumValues(values: List[Symbol], config: Seq[MacroDeriveEnumSetting]) = {
    ...
    values.filter(m =>
      actualFields.contains(m.name.decodedName.toString.trim) && !memberExcluded(m.annotations))
  }

  private def validateEnumValueConfig(
      knownMembers: List[Symbol],
      config: Seq[MacroDeriveEnumSetting]) = {
  ...
  private def validateEnumConfig(config: Seq[Tree]) = config.map {
    ...
    case tree @ q"$setting.apply(${value: String}, $graphqlName)"
        if checkSetting[RenameValue.type](setting) =>
      Right(MacroRenameValue(value, graphqlName, tree.pos))

    case tree @ q"$setting.apply(${value: String}, $deprecationReason)"
        if checkSetting[DeprecateValue.type](setting) =>
      Right(MacroDeprecateValue(value, q"Some($deprecationReason)", tree.pos))

    case tree @ q"$setting.apply(..${values: List[String]})"
        if checkSetting[IncludeValues.type](setting) =>
      Right(MacroIncludeValues(values.toSet, tree.pos))

    case tree @ q"$setting.apply(..${values: List[String]})"
        if checkSetting[ExcludeValues.type](setting) =>
  ...
  case class MacroDeprecateValue(value: String, deprecationReason: Tree, pos: Position)
      extends MacroDeriveEnumSetting
  case class MacroRenameValue(value: String, graphqlName: Tree, pos: Position)
      extends MacroDeriveEnumSetting

  case class MacroIncludeValues(values: Set[String], pos: Position) extends MacroDeriveEnumSetting
  case class MacroExcludeValues(fieldNames: Set[String], pos: Position)
      extends MacroDeriveEnumSetting
