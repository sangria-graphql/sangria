...
@implicitNotFound(
  msg =
    "Can't find suitable GraphQL output type for ${T}. If you have defined it already, please consider making it implicit and ensure that it's available in the scope.")
...
object GraphQLOutputTypeLookup extends GraphQLOutputTypeLookupLowPrio {
  implicit def outLookup[T](implicit out: OutputType[T]): GraphQLOutputTypeLookup[T] =
  ...
  implicit def optionLookup[T: GraphQLOutputTypeLookup]: GraphQLOutputTypeLookup[Option[T]] =
    new GraphQLOutputTypeLookup[Option[T]] {
      override val graphqlType: OptionType[T] = OptionType(
        implicitly[GraphQLOutputTypeLookup[T]].graphqlType)
    }
}
