...
//  )
//
//  val OperationNameExtension: Middleware[Any, Any] = Middleware.simpleExtension[Any, Any](ctx =>
//    ast.ObjectValue(
//      "batch" -> ast.ObjectValue("operationName" ->
//        ctx.operationName.fold(ast.NullValue(): ast.Value)(ast.StringValue(_)))))
//
//  /** __EXPERIMENTAL__ */
//  def executeBatch[Ctx, Root, Input, T, F[_]: Effect](
...
//      executor: Executor[Ctx, Root, Future],
//      queryAst: ast.Document,
