...
trait Ignored extends PositionTracking { this: Parser ⇒
  ...
  def Comments = rule {
    test(parseComments) ~ CommentCap.* ~ Ignored.* ~> (_.toVector) | CommentNoCap.* ~ Ignored.* ~ push(Vector.empty)
  }
...
trait Document {
  this: Parser with Operations with Ignored with Fragments with Operations with Values with TypeSystemDefinitions ⇒
  ...
  def ObjectTypeExtensionDefinition = rule {
    (Comments ~ trackPos ~ extend ~ `type` ~ Name ~ (ImplementsInterfaces.? ~> (_ getOrElse Vector.empty)) ~ (DirectivesConst.? ~> (_ getOrElse Vector.empty)) ~ FieldsDefinition ~> (
      (
        ...
        dirs,
        fields
      ) ⇒ ast.ObjectTypeExtensionDefinition(name, interfaces, fields._1.toVector, dirs, comment, fields._2, location)
    )) |
      (Comments ~ trackPos ~ extend ~ `type` ~ Name ~ (ImplementsInterfaces.? ~> (_ getOrElse Vector.empty)) ~ DirectivesConst ~> (
        (
          ...
          interfaces,
          dirs
        ) ⇒ ast.ObjectTypeExtensionDefinition(name, interfaces, Vector.empty, dirs, comment, Vector.empty, location)
      )) |
  ...
  def InterfaceTypeExtensionDefinition = rule {
    (Comments ~ trackPos ~ extend ~ interface ~ Name ~ (DirectivesConst.? ~> (_ getOrElse Vector.empty)) ~ FieldsDefinition ~> (
      (
        ...
        dirs,
        fields
      ) ⇒ ast.InterfaceTypeExtensionDefinition(name, fields._1.toVector, dirs, comment, fields._2, location)
    )) |
      (Comments ~ trackPos ~ extend ~ interface ~ Name ~ DirectivesConst ~> (
        (
          ...
          name,
          dirs
        ) ⇒ ast.InterfaceTypeExtensionDefinition(name, Vector.empty, dirs, comment, Vector.empty, location)
      ))
  }
  ...
  def EnumTypeExtensionDefinition = rule {
    (Comments ~ trackPos ~ extend ~ enum ~ Name ~ (DirectivesConst.? ~> (_ getOrElse Vector.empty)) ~ EnumValuesDefinition ~> (
      (
        ...
        dirs,
        values
      ) ⇒ ast.EnumTypeExtensionDefinition(name, values._1.toVector, dirs, comment, values._2, location)
    )) |
      (Comments ~ trackPos ~ extend ~ enum ~ Name ~ DirectivesConst ~> (
        (
          ...
          name,
          dirs
        ) ⇒ ast.EnumTypeExtensionDefinition(name, Vector.empty, dirs, comment, Vector.empty, location)
      ))
  }
  ...
  def InputObjectTypeExtensionDefinition = rule {
    (Comments ~ trackPos ~ extend ~ inputType ~ Name ~ (DirectivesConst.? ~> (_ getOrElse Vector.empty)) ~ InputFieldsDefinition ~> (
      (
        ...
        dirs,
        fields
      ) ⇒ ast.InputObjectTypeExtensionDefinition(name, fields._1.toVector, dirs, comment, fields._2, location)
    )) |
      (Comments ~ trackPos ~ extend ~ inputType ~ Name ~ DirectivesConst ~> (
        (
          ...
          name,
          dirs
        ) ⇒ ast.InputObjectTypeExtensionDefinition(name, Vector.empty, dirs, comment, Vector.empty, location)
      ))
  }
...
trait Operations extends PositionTracking {
  ...
  def OperationDefinition = rule {
    Comments ~ trackPos ~ SelectionSet ~> (
      (
        ...
        location,
        s
      ) ⇒ ast.OperationDefinition(selections = s._1, comments = comment, trailingComments = s._2, location = location)
    ) |
  ...
  def VariableDefinition = rule {
    Comments ~ trackPos ~ Variable ~ ws(':') ~ Type ~ DefaultValue.? ~>
      (
        (
          comment,
          location,
          ...
          tpe,
          defaultValue
        ) ⇒ ast.VariableDefinition(name, tpe, defaultValue, comment, location)
      )
  }
    ...
    Comments ~ trackPos ~ Fragment ~ FragmentName ~ ExperimentalFragmentVariables ~ TypeCondition ~ (Directives.? ~> (_ getOrElse Vector.empty)) ~ SelectionSet ~>
      (
        (
          ...
          dirs,
          sels
        ) ⇒ ast.FragmentDefinition(name, typeCondition, dirs, sels._1, vars, comment, sels._2, location)
      )
  }
...
trait Values { this: Parser with Tokens with Ignored with Operations ⇒
  ...
  def EnumValue = rule {
    Comments ~ !(True | False) ~ trackPos ~ Name ~> ((comment, location, name) ⇒ ast.EnumValue(name, comment, location))
  }
...
object QueryParser {
  ...
  def parseInput(input: ParserInput)(implicit scheme: DeliveryScheme[ast.Value]): scheme.Result = {
    ...
    parser.InputDocument.run() match {
      ...
      case Success(res) ⇒
        scheme.failure(new IllegalArgumentException("Input document does not contain any value definitions."))
      case Failure(e: ParseError) ⇒ scheme.failure(SyntaxError(parser, input, e))
      case Failure(e) ⇒ scheme.failure(e)
    }
  }

  def parseInputDocument(input: String, config: ParserConfig = ParserConfig.default)(
