...
package object introspection {
  ...
  val __DirectiveLocation = EnumType(
    ...
    List(
      ...
      EnumValue("FIELD", value = DirectiveLocation.Field, description = Some("Location adjacent to a field.")),
      EnumValue(
        "FRAGMENT_DEFINITION",
      ...
      EnumValue("ENUM", value = DirectiveLocation.Enum, description = Some("Location adjacent to an enum definition.")),
      EnumValue(
        "ENUM_VALUE",
        value = DirectiveLocation.EnumValue,
        description = Some("Location adjacent to an enum value definition.")
      ),
      EnumValue("INPUT_OBJECT", value = DirectiveLocation.InputObject, description = Some("INPUT_OBJECT")),
      EnumValue(
  ...
  val __Type: ObjectType[Unit, (Boolean, Type)] = ObjectType(
    ...
    fieldsFn = () ⇒
      List[Field[Unit, (Boolean, Type)]](
        ...
        Field("fields", OptionType(ListType(__Field)), arguments = includeDeprecated :: Nil, resolve = ctx ⇒ {
          ...
          tpe match {
            ...
            case t: ObjectLikeType[_, _] ⇒
              Some(t.uniqueFields.asInstanceOf[Vector[Field[_, _]]].filter(_.deprecationReason.isEmpty))
            case _ ⇒ None
          }
        }),
        ...
        Field("enumValues", OptionType(ListType(__EnumValue)), arguments = includeDeprecated :: Nil, resolve = ctx ⇒ {
          val incDep = ctx.arg(includeDeprecated)
