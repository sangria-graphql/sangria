...
object IntrospectionParser {
  def parse[In](
    introspectionResult: In
  )(implicit iu: InputUnmarshaller[In], scheme: DeliveryScheme[IntrospectionSchema]): scheme.Result =
    try {
  ...
  private def parseField[In : InputUnmarshaller](field: In, path: Vector[String]) =
    IntrospectionField(
      ...
      args = mapFieldOpt(field, "args") map um.getListValue getOrElse Vector.empty map (
        arg ⇒ parseInputValue(arg, path :+ "args")
      ),
  ...
  private def parseObject[In : InputUnmarshaller](tpe: In, path: Vector[String]) =
    IntrospectionObjectType(
      ...
      fields = mapFieldOpt(tpe, "fields") map um.getListValue getOrElse Vector.empty map (
        field ⇒ parseField(field, path :+ "fields")
      ),
      interfaces = mapFieldOpt(tpe, "interfaces") map um.getListValue getOrElse Vector.empty map (
        i ⇒ parseNamedTypeRef(i, path :+ "interfaces")
      )
    )
  ...
  private def parseInterface[In : InputUnmarshaller](tpe: In, path: Vector[String]) =
    IntrospectionInterfaceType(
      ...
      possibleTypes = mapFieldOpt(tpe, "possibleTypes") map um.getListValue getOrElse Vector.empty map (
        i ⇒ parseNamedTypeRef(i, path :+ "possibleTypes")
      )
    )
  ...
  private def parseInputObject[In : InputUnmarshaller](tpe: In, path: Vector[String]) =
    IntrospectionInputObjectType(
      name = mapStringField(tpe, "name", path),
      description = mapStringFieldOpt(tpe, "description", path),
      inputFields = mapFieldOpt(tpe, "inputFields") map um.getListValue getOrElse Vector.empty map (
        arg ⇒ parseInputValue(arg, path :+ "inputFields")
      )
    )
  ...
  private def parseDirective[In : InputUnmarshaller](directive: In, path: Vector[String]) =
    IntrospectionDirective(
      ...
      locations = um
        ...
        .toSet,
      args = mapFieldOpt(directive, "args") map um.getListValue getOrElse Vector.empty map (
        arg ⇒ parseInputValue(arg, path :+ "args")
      )
    )
  ...
  private def parseSchema[In : InputUnmarshaller](schema: In, path: Vector[String]) =
    IntrospectionSchema(
      ...
      directives = mapFieldOpt(schema, "directives") map um.getListValue getOrElse Vector.empty map (
        i ⇒ parseDirective(i, path :+ "directives")
      )
    )
