...
case class ScalarAlias[T, ST](aliasFor: ScalarType[ST], toScalar: T ⇒ ST, fromScalar: ST ⇒ Either[Violation, T])
    extends InputType[T @@ CoercedScalaResult]
    with OutputType[T]
    with LeafType
    with NullableType
...
sealed trait ObjectLikeType[Ctx, Val]
  ...
  lazy val fields: Vector[Field[Ctx, _]] = ownFields ++ interfaces.flatMap(
    i ⇒ i.fields.asInstanceOf[Vector[Field[Ctx, _]]]
  )
...
object PossibleInterface extends PossibleInterfaceLowPrioImplicits {
  implicit def apply[Ctx, Abstract, Concrete](
    interface: InterfaceType[Ctx, Abstract]
  )(implicit ev: PossibleType[Abstract, Concrete]): PossibleInterface[Ctx, Concrete] =
    PossibleInterface[Ctx, Concrete](interface)
}

trait PossibleInterfaceLowPrioImplicits {
  implicit def applyUnit[Ctx, Abstract, Concrete](
    interface: InterfaceType[Ctx, Abstract]
  )(implicit ev: PossibleType[Abstract, Concrete]): PossibleInterface[Unit, Concrete] =
...
object PossibleObject {
  implicit def apply[Ctx, Abstract, Concrete](
    obj: ObjectType[Ctx, Concrete]
  )(implicit ev: PossibleType[Abstract, Concrete]): PossibleObject[Ctx, Abstract] =
    PossibleObject[Ctx, Abstract](obj)

  implicit def applyUnit[Ctx, Abstract, Concrete](
    obj: ObjectType[Unit, Concrete]
  )(implicit ev: PossibleType[Abstract, Concrete]): PossibleObject[Ctx, Abstract] =
...
object PossibleType {
  ...
  implicit def InheritanceBasedPossibleType[Abstract, Concrete](
    implicit ev: Concrete <:< Abstract
  ): PossibleType[Abstract, Concrete] =
...
case class UnionType[Ctx](
  ...
  def mapValue[T](func: T ⇒ Any): OutputType[T] =
    new UnionType[Ctx](name, description, types, astDirectives, astNodes) with MappedAbstractType[T] {
...
object Argument {
  def apply[T, Default](name: String, argumentType: InputType[T], description: String, defaultValue: Default)(
    implicit toInput: ToInput[Default, _],
  ...
  )(implicit toInput: ToInput[Default, _], fromInput: FromInput[T], res: ArgumentType[T]): Argument[res.Res] =
    Argument(name, argumentType, None, Some(defaultValue → toInput), fromInput, Vector.empty, Vector.empty)

  def apply[T](name: String, argumentType: InputType[T], description: String)(
  ...
  def createWithoutDefault[T](name: String, argumentType: InputType[T], description: Option[String])(
    implicit fromInput: FromInput[T],
    res: ArgumentType[T]
  ): Argument[res.Res] =
...
case class EnumType[T](
  ...
  def coerceUserInput(value: Any): Either[Violation, (T, Boolean)] = value match {
    case valueName: String ⇒
      byName get valueName map (v ⇒ Right(v.value → v.deprecationReason.isDefined)) getOrElse Left(
        EnumValueCoercionViolation(valueName, name, values.map(_.name))
      )
...
object InputObjectType {
  ...
  def apply[T](name: String, fields: List[InputField[_]])(
    implicit res: InputObjectDefaultResult[T]
  ): InputObjectType[res.Res] =
    InputObjectType(name, None, fieldsFn = () ⇒ fields, Vector.empty, Vector.empty)
  def apply[T](name: String, description: String, fields: List[InputField[_]])(
    implicit res: InputObjectDefaultResult[T]
  ): InputObjectType[res.Res] =
    InputObjectType(name, Some(description), fieldsFn = () ⇒ fields, Vector.empty, Vector.empty)

  def apply[T](name: String, fieldsFn: () ⇒ List[InputField[_]])(
    implicit res: InputObjectDefaultResult[T]
  ): InputObjectType[res.Res] =
    InputObjectType(name, None, fieldsFn, Vector.empty, Vector.empty)
  def apply[T](name: String, description: String, fieldsFn: () ⇒ List[InputField[_]])(
    implicit res: InputObjectDefaultResult[T]
  ): InputObjectType[res.Res] =
...
object InputField {
  def apply[T, Default](name: String, fieldType: InputType[T], description: String, defaultValue: Default)(
    implicit toInput: ToInput[Default, _],
    res: WithoutInputTypeTags[T]
  ): InputField[res.Res] =
    InputField(name, fieldType, Some(description), Some(defaultValue → toInput), Vector.empty, Vector.empty)
      .asInstanceOf[InputField[res.Res]]

  def apply[T, Default](name: String, fieldType: InputType[T], defaultValue: Default)(
    implicit toInput: ToInput[Default, _],
    res: WithoutInputTypeTags[T]
  ): InputField[res.Res] =
  ...
  def apply[T](name: String, fieldType: InputType[T], description: String)(
    implicit res: WithoutInputTypeTags[T]
  ): InputField[res.Res] =
  ...
  def createFromMacroWithDefault[T, Default](
    ...
    InputField(name, fieldType, description, Some(defaultValue → toInput), Vector.empty, Vector.empty)
      .asInstanceOf[InputField[res.Res]]

  def createFromMacroWithoutDefault[T](name: String, fieldType: InputType[T], description: Option[String])(
    implicit res: WithoutInputTypeTags[T]
  ): InputField[res.Res] =
...
case class Schema[Ctx, Val](
  ...
  lazy val types: Map[String, (Int, Type with Named)] = {
    ...
    def collectTypes(
      ...
      tpe: Type,
      result: Map[String, (Int, Type with Named)]
    ): Map[String, (Int, Type with Named)] = {
      tpe match {
        case null ⇒
        ...
        case t: Named if result contains t.name ⇒
          result get t.name match {
            case Some(found)
                if !sameType(found._2, t) && t.isInstanceOf[ScalarAlias[_, _]] && found._2.isInstanceOf[ScalarType[_]] ⇒
              result
          ...
          val own = t.fields.foldLeft(updated(priority, t.name, t, result, parentInfo)) {
            case (acc, field) ⇒
              val fromArgs = field.arguments.foldLeft(
                collectTypes(s"a field '${field.name}' of '${t.name}' type", priority, field.fieldType, acc)
              ) {
                case (aacc, arg) ⇒
                  collectTypes(
              ...
              field.manualPossibleTypes().foldLeft(fromArgs) {
                case (acc, objectType) ⇒
                  collectTypes(s"a manualPossibleType defined in '${t.name}' type", priority, objectType, acc)
              }
          }
          ...
          val withPossible = t match {
            case i: InterfaceType[_, _] ⇒
              i.manualPossibleTypes().foldLeft(own) {
                case (acc, objectType) ⇒
                  collectTypes(s"a manualPossibleType defined in '${i.name}' type", priority, objectType, acc)
              }
            case _ ⇒ own
          }
        ...
        case t @ UnionType(name, _, types, _, _) ⇒
          types.foldLeft(updated(priority, name, t, result, parentInfo)) {
            case (acc, tpe) ⇒ collectTypes(s"a '$name' type", priority, tpe, acc)
          }
      }
    }
    ...
    val queryTypesWithAdditions = additionalTypes.foldLeft(queryTypes) {
      case (acc, tpe) ⇒ collectTypes("additional type", 10, tpe, acc)
    }
