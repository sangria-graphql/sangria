...
class CachedCheck {
  ...
  def checkFieldsInSetCanMerge(fields: FieldSet, builder: SelectionConflictViolationsBuilder): Unit = {
    getCacheLine(fields).checkFieldsInSetCanMerge(builder)
  }

  private def getCacheLine(fields: FieldSet): FieldSetCache = {
    cache.computeIfAbsent(fields, new function.Function[FieldSet, FieldSetCache] {
            ...
            field.outputTypeShape match {
              ...
              case knownShape: TypeShape.Known =>
                fieldsWithKnownResponseShapes
                  .computeIfAbsent(knownShape, new function.Function[TypeShape.Known, util.ArrayList[SelectionField]] {
                    override def apply(key: TypeShape.Known): util.ArrayList[SelectionField] = new util.ArrayList()
                  })
                  .add(field)
            }
          }
        ...
        new Consumer[SelectionField] {
          override def accept(field: SelectionField): Unit = {
            fieldsWithSameNameAndArguments
              .computeIfAbsent(field.fieldNameAndArguments, new function.Function[FieldNameAndArguments, util.ArrayList[SelectionField]] {
                override def apply(key: FieldNameAndArguments): util.ArrayList[SelectionField] = new util.ArrayList()
              })
              .add(field)
          }
        }
      ...
      if (cacheGroupByOutputNames == null) {
        ...
        fields.forEach {
          new Consumer[SelectionField] {
            override def accept(field: SelectionField): Unit = {
              outputNames
                .computeIfAbsent(field.outputName, new function.Function[OutputName, FieldSetBuilder] {
              ...
              field.parentTypeAbstractness match {
                ...
                case concrete: TypeAbstractness.Concrete =>
                  fieldsWithConreteParents
                    .computeIfAbsent(concrete, new function.Function[TypeAbstractness.Concrete, FieldSetBuilder] {
                      override def apply(t: TypeAbstractness.Concrete): FieldSetBuilder = newFieldSetBuilder()
                    })
                    .add(field)
              }
            }
