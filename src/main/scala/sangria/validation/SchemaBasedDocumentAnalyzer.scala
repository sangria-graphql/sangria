...
case class SchemaBasedDocumentAnalyzer(schema: Schema[_, _], document: ast.Document) {
  ...
  lazy val deprecatedUsages: Vector[DeprecatedUsage] =
    AstVisitor
      .visitAstWithState(schema, document, MutableMap[String, DeprecatedUsage]()) { (typeInfo, deprecated) ⇒
        AstVisitor.simple {
  ...
  lazy val introspectionUsages: Vector[IntrospectionUsage] =
    AstVisitor
      .visitAstWithState(schema, document, MutableMap[String, IntrospectionUsage]()) { (typeInfo, usages) ⇒
        AstVisitor.simple {
...
object SchemaBasedDocumentAnalyzer {
  ...
  case class IntrospectionUsage(parentType: CompositeType[_], field: Field[_, _], astField: ast.Field)
      extends Violation {
