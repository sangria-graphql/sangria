...
case class EnumValueCoercionViolation(name: String, typeName: String, knownValues: Seq[String])
    extends ValueCoercionViolation(
      s"Enum value '$name' is undefined in enum type '$typeName'. Known values are: ${knownValues mkString ", "}."
    )
...
case class InputObjectTypeMismatchViolation(
  ...
  lazy val simpleErrorMessage =
    s"Value '$value' of wrong type was provided to the field of type '$typeName' at path '${fieldPath mkString "."}'."
}
...
case class UnknownDirectiveViolation(name: String, sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation
    with SpecViolation {
...
case class MisplacedDirectiveViolation(
  ...
  val args = Map(
    "directiveName" → name,
    "location" → correctPlacement.map(loc ⇒ DirectiveLocation.toSpecString(loc._1)).getOrElse("here")
  )

  lazy val simpleErrorMessage = s"Directive '$name' may not be used ${correctPlacement.fold("here")("on " + _._2)}."
}

case class UnknownFragmentViolation(name: String, sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
...
case class CycleErrorViolation(
  ...
  lazy val simpleErrorMessage = s"Cannot spread fragment '$fragmentName' within itself${if (spreadNames.nonEmpty)
...
case class UndefinedVarViolation(varName: String, sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
  lazy val simpleErrorMessage = s"Variable '$$$varName' is not defined."
}

case class UnusedFragmentViolation(name: String, sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
...
case class FieldsConflictViolation(
  ...
  private def reasonMessage(reason: Either[String, Vector[ConflictReason]]): String = reason match {
    ...
    case Right(subReasons) ⇒
      subReasons map (sr ⇒ s"subfields '${sr.fieldName}' conflict because ${reasonMessage(sr.reason)}") mkString " and "
  }
}

case class AnonOperationNotAloneViolation(sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
...
case class DuplicateArgNameViolation(argName: String, sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
  lazy val simpleErrorMessage = s"There can be only one argument named '$argName'."
}

case class DuplicateInputFieldViolation(name: String, sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
...
case class NotAllSubscriptionFieldsViolation(typeName: String, fieldNames: Vector[String]) extends Violation {
  lazy val errorMessage =
    s"Subscription type '$typeName' may either contain only non-subscription fields or only subscription fields (defined with `Field.subs`). Following fields are non-subscription fields among other subscription fields: ${fieldNames map ("'" + _ + "'") mkString ", "}."
}

case class NotAllSubscriptionFieldsHaveSameStreamViolation(typeName: String, fieldNames: Vector[String])
    extends Violation {
  lazy val errorMessage =
    s"Some fields of subscription type '$typeName' have incompatible stream implementations: ${fieldNames map ("'" + _ + "'") mkString ", "}."
}
...
case class RequiredFieldViolation(
  ...
  lazy val simpleErrorMessage = s"Field '$typeName.$fieldName' of required type '$fieldType' was not provided."
}
...
case class UnknownFieldViolation(
  ...
  lazy val simpleErrorMessage = s"Field '$fieldName' is not defined by type '$typeName'${message.fold(".")("; " + _)}"
}

case class NotNullValueIsNullViolation(sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
...
case class GenericInvalidValueViolation(sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
...
case class ConflictingTypeDefinitionViolation(typeName: String, conflictingTypes: List[String], parentInfo: String)
    extends Violation {
  lazy val errorMessage =
    s"Type name '$typeName' is used for several conflicting GraphQL type kinds: ${conflictingTypes mkString ", "}. Conflict found in $parentInfo."
}
...
case class ReservedTypeNameViolation(typeName: String, sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
  lazy val simpleErrorMessage =
...
case class EmptyInputFieldsViolation(typeName: String, sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
...
case class NonUniqueSchemaDefinitionViolation(sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
...
case class ExistingTypeViolation(typeName: String, sourceMapper: Option[SourceMapper], locations: List[AstLocation])
    extends AstNodeViolation {
  lazy val simpleErrorMessage =
