...
object SchemaRenderer {
  ...
  def renderEnumValue(v: EnumValue[_]) =
    ast.EnumValueDefinition(
      v.name,
      withoutDeprecated(v.astDirectives) ++ renderDeprecation(v.deprecationReason.isDefined, v.deprecationReason),
      renderDescription(v.description)
    )
  ...
  private def renderSchemaDefinition(schema: IntrospectionSchema): Option[ast.SchemaDefinition] =
    if (isSchemaOfCommonNames(
      ...
      val withMutation = schema.mutationType.fold(withQuery)(
        t ⇒ withQuery :+ ast.OperationTypeDefinition(ast.OperationType.Mutation, ast.NamedType(t.name))
      )
      val withSubs = schema.subscriptionType.fold(withMutation)(
        t ⇒ withMutation :+ ast.OperationTypeDefinition(ast.OperationType.Subscription, ast.NamedType(t.name))
      )
  ...
  private def renderSchemaDefinition(schema: Schema[_, _]): Option[ast.SchemaDefinition] =
    if (isSchemaOfCommonNames(schema.query.name, schema.mutation.map(_.name), schema.subscription.map(_.name)))
      ...
      val withMutation = schema.mutation.fold(withQuery)(
        t ⇒ withQuery :+ ast.OperationTypeDefinition(ast.OperationType.Mutation, ast.NamedType(t.name))
      )
      val withSubs = schema.subscription.fold(withMutation)(
        t ⇒ withMutation :+ ast.OperationTypeDefinition(ast.OperationType.Subscription, ast.NamedType(t.name))
      )
  ...
  def schemaAstFromIntrospection(
    ...
    filter: SchemaFilter = SchemaFilter.default
  ): ast.Document = {
...
object SchemaFilter {
  val withoutSangriaBuiltIn: SchemaFilter =
    SchemaFilter(typeName ⇒ !Schema.isBuiltInType(typeName), dirName ⇒ !Schema.isBuiltInDirective(dirName))

  val default: SchemaFilter = withoutSangriaBuiltIn

  val withoutGraphQLBuiltIn =
    SchemaFilter(typeName ⇒ !Schema.isBuiltInGraphQLType(typeName), dirName ⇒ !Schema.isBuiltInDirective(dirName))
  ...
  val introspection: SchemaFilter =
    SchemaFilter(typeName ⇒ Schema.isIntrospectionType(typeName), Function.const(false), renderSchema = false)
