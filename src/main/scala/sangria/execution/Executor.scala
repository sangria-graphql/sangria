...
case class Executor[Ctx, Root](
  ...
  def execute[Input](
    ...
    implicit marshaller: ResultMarshaller,
  ...
  private def executeOperation[Input](
    ...
    try {
      ...
      val result =
        operation.operationType match {
          ...
          case ast.OperationType.Mutation ⇒
            resolver.resolveFieldsSeq(tpe, root, fields)(scheme).asInstanceOf[scheme.Result[Ctx, marshaller.Node]]
      ...
      if (middlewareVal.nonEmpty)
        scheme.onComplete(result)(middlewareVal foreach {
          case (v, m) ⇒ m.afterQuery(v.asInstanceOf[m.QueryVal], middlewareCtx)
        })
      else result
    } catch {
      case NonFatal(error) ⇒
...
object Executor {
  ...
  def getOperation(
    ...
    if (document.operations.size != 1 && operationName.isEmpty)
      Failure(
        OperationSelectionError("Must provide operation name if query contains multiple operations", exceptionHandler)
      )
    else {
      val unexpectedDefinition = document.definitions.find(
        d ⇒ !(d.isInstanceOf[ast.OperationDefinition] || d.isInstanceOf[ast.FragmentDefinition])
      )
...
class PreparedQuery[Ctx, Root, Input] private[execution] (
  ...
  def execute(
    ...
    root: Root = root
  )(implicit marshaller: ResultMarshaller, scheme: ExecutionScheme): scheme.Result[Ctx, marshaller.Node] =
