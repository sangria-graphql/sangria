...
object QueryReducer {
  ...
  def rejectMaxDepth[Ctx](maxDepth: Int): QueryReducer[Ctx, Ctx] =
    measureDepth[Ctx]((depth, ctx) ⇒ if (depth > maxDepth) throw new MaxQueryDepthReachedError(maxDepth) else ctx)

  def collectTags[Ctx, T](
...
class TagCollector[Ctx, T](tagMatcher: PartialFunction[FieldTag, T], action: (Seq[T], Ctx) ⇒ ReduceAction[Ctx, Ctx])
...
class HasIntrospectionReducer[Ctx](includeTypeName: Boolean, action: (Boolean, Ctx) ⇒ ReduceAction[Ctx, Ctx])
    extends QueryReducer[Ctx, Ctx] {
  type Acc = Boolean
