...
class FieldCollector[Ctx, Val](
  ...
  def collectFields(
    ...
    selections: Vector[ast.SelectionContainer]
  ): Try[CollectedFields] =
        ...
        selection match {
          ...
          case fragment @ ast.InlineFragment(_, dirs, fragmentSelections, _, _, _) ⇒
            for {
              ...
              fragmentFields ← if (shouldInclude && fragmentConditionMatch)
  ...
  def shouldIncludeNode(directives: Vector[ast.Directive], selection: ast.WithDirectives): Try[Boolean] = {
    val possibleDirs = directives
              ...
              dd ⇒
                selection match {
                  ...
                  case op: ast.OperationDefinition
                      if op.operationType == OperationType.Query && !dd.locations.contains(DirectiveLocation.Query) ⇒
                    Failure(
                      new ExecutionError(
                  ...
                  case op: ast.OperationDefinition
                      if op.operationType == OperationType.Mutation && !dd.locations
                        .contains(DirectiveLocation.Mutation) ⇒
                    Failure(
                      new ExecutionError(
                  ...
                  case op: ast.OperationDefinition
                      if op.operationType == OperationType.Subscription && !dd.locations
                        .contains(DirectiveLocation.Subscription) ⇒
                    Failure(
                      new ExecutionError(
      ...
      .map(_.flatMap {
        case (astDir, dir) ⇒
          valueCollector.getArgumentValues(Some(astDir), dir.arguments, astDir.arguments, variables) map (dir → _)
      })
