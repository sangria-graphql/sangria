...
class Resolver[Ctx](
  ...
  def resolveFieldsPar(tpe: ObjectType[Ctx, _], value: Any, fields: CollectedFields)(
    ...
    handleScheme(
      processFinalResolve(resolveActionsPar(ExecutionPath.empty, tpe, actions, userContext, fields.namesOrdered)) map (_ → userContext),
      scheme
    )
  }
  ...
  def resolveFieldsSubs(tpe: ObjectType[Ctx, _], value: Any, fields: CollectedFields)(
    ...
    scheme match {
      case ExecutionScheme.Default ⇒
        val (s, res) =
          resolveSubs[({ type X[Y] })#X](ExecutionPath.empty, tpe, value, fields, ErrorRegistry.empty, None)
      ...
      case ExecutionScheme.Extended ⇒
        ...
        s.first(res)
          .map {
            case (errors, res) ⇒
              ExecutionResult(userContext, res, errors, middleware, validationTiming, queryReducerTiming)
          }
          .asInstanceOf[scheme.Result[Ctx, marshaller.Node]]

      case es: ExecutionScheme.StreamBasedExecutionScheme[({ type X[Y] })#X @unchecked] ⇒
        val (_, res) =
          resolveSubs(ExecutionPath.empty, tpe, value, fields, ErrorRegistry.empty, Some(es.subscriptionStream))

        es.subscriptionStream
          .map(res) {
            case (errors, r) if es.extended ⇒
              ExecutionResult(userContext, r, errors, middleware, validationTiming, queryReducerTiming)
            case (_, r) ⇒ r
          }
          .asInstanceOf[scheme.Result[Ctx, marshaller.Node]]
  ...
  def handleScheme(
    ...
    case ExecutionScheme.Extended ⇒
      result
        .map {
          case ((errors, res), uc) ⇒ ExecutionResult(uc, res, errors, middleware, validationTiming, queryReducerTiming)
        }
        .asInstanceOf[scheme.Result[Ctx, marshaller.Node]]
  ...
  private def immediatelyResolveDeferred[T](uc: Ctx, dr: DeferredResult, fn: Future[Result] ⇒ Future[T]): Future[T] = {
    ...
    resolveDeferredWithGrouping(dr.deferred).foreach(groups ⇒ groups.foreach(group ⇒ resolveDeferred(uc, group)))
  ...
  private type Actions = (
    ErrorRegistry,
    Option[
      Vector[(Vector[ast.Field], Option[(Field[Ctx, _], Option[MappedCtxUpdate[Ctx, Any, Any]], LeafAction[Ctx, _])])]
    ]
  )
  ...
  def resolveSubs[S[_]](
    ...
    val firstStream =
      tpe.uniqueFields.head.tags.collectFirst { case SubscriptionField(s) ⇒ s }.get.asInstanceOf[SubscriptionStream[S]]
          ...
          marshallResult(
            Result(
              ...
              if (isOptional(tpe, origField.name))
                Some(marshaller.addMapNodeElem(resMap, origField.outputName, marshaller.nullNode, optional = true))
              else None
            )
          )
        )
    ...
    stream → stream.mapFuture(stream.merge(fieldStreams.asInstanceOf[Vector[S[Result]]]))(
      r ⇒ processFinalResolve(r.buildValue)
    )
  }
  ...
  def resolveSeq(
    ...
    fields.fields.foldLeft(
      Future.successful((Result(ErrorRegistry.empty, Some(marshaller.emptyMapNode(fields.namesOrdered))), userContext))
    ) {
      case (future, elem) ⇒
        future.flatMap { resAndCtx ⇒
          (resAndCtx, elem) match {
            case (acc @ (Result(_, None, _), _), _) ⇒ Future.successful(acc)
            case (acc, CollectedField(name, origField, _)) if tpe.getField(schema, origField.name).isEmpty ⇒
        ...
        result match {
          ...
          case FutureValue(f) ⇒
            f.map { v ⇒
              ...
              .recover {
                case e ⇒
                  Result(ErrorRegistry(path.add(origField, tpe), resolveError(e), fields.head.location), None) → uc
              }
  ...
  private def calcComplexity(path: ExecutionPath, astField: ast.Field, field: Field[Ctx, _], uc: Ctx) = {
    val args =
      valueCollector.getFieldArgumentValues(path, Some(astField), field.arguments, astField.arguments, variables)
      ...
      case ((errors, s @ Some(acc)), CollectedField(name, origField, Success(fields))) ⇒
        resolveField(userCtx, tpe, path.add(origField, tpe), value, errors, name, fields) match {
          case StandardFieldResolution(updatedErrors, result, updateCtx) ⇒
            updatedErrors → Some(acc :+ (fields → Some((tpe.getField(schema, origField.name).head, updateCtx, result))))
          case ErrorFieldResolution(updatedErrors) if isOptional(tpe, origField.name) ⇒
  ...
  private def resolveActionSequenceValues(
    ...
    actions.map {
      ...
      case PartialFutureValue(future) ⇒
        SeqRes(
          future.map { case PartialValue(v, es) ⇒ SeqFutRes(v, es) }.recover { case e ⇒ SeqFutRes(errors = Vector(e)) }
        )
      case DeferredValue(deferred) ⇒
        ...
        SeqRes(promise.future map { case (dctx, v, es) ⇒ SeqFutRes(v, es, dctx) } recover {
          case e ⇒ SeqFutRes(errors = Vector(e))
        }, defer)
      ...
      case SubscriptionValue(_, _) ⇒
        SeqRes(
          SeqFutRes(
            errors = Vector(new IllegalStateException("Subscription values are not supported for normal operations"))
          )
        )
    }
      ...
      case Some(results) ⇒
        ...
        val resSoFar = simpleRes.foldLeft(Result(errors, Some(marshaller.emptyMapNode(fieldsNamesOrdered)))) {
          case (res, (astField, other)) ⇒
            res addToMap (other, astField.outputName, isOptional(tpe, astField.name), path.add(astField, tpe), astField.location, res.errors)
        }
        ...
        if (complexRes.isEmpty) resSoFar.buildValue
          ...
          val finalValue = Future.sequence(complexRes.map {
            case (astField, DeferredResult(_, future)) ⇒ future map (astField → _)
          }) map { results ⇒
            results
              .foldLeft(resSoFar) {
                case (res, (astField, other)) ⇒
  ...
  private def resolveDeferred(uc: Ctx, toResolve: Vector[Defer]) =
    if (toResolve.nonEmpty) {
      ...
      try {
        val resolved =
          deferredResolver.resolve(toResolve map (d ⇒ findActualDeferred(d.deferred)), uc, deferredResolverState)
        ...
        if (toResolve.size == resolved.size) {
          ...
          for (i ← toResolve.indices) {
            ...
            toRes.promise tryCompleteWith mapAllDeferred(toRes.deferred, resolved(i))
        ...
        if (isUndefinedValue(value))
          ...
          if (simpleRes.size == res.size)
            ...
            DeferredResult(
              ...
              futureValue = Future.sequence(resultFutures.result()) map (resolveSimpleListValue(
                _,
                path,
                optional,
  ...
  def resolveField(
    ...
    maxQueryDepth match {
      case Some(max) if path.size > max ⇒
        ErrorFieldResolution(errors.add(path, new MaxQueryDepthReachedError(max), astField.location))
      case _ ⇒
                ...
                mAfter.foldLeft(v) {
                  case (acc, (BeforeFieldResult(cv, _, _), mv, m: MiddlewareAfterField[Ctx])) ⇒
                    m.afterField(
                      ...
                      .asInstanceOf[Option[Val]] getOrElse acc
                  case (acc, _) ⇒ acc
                }
                ...
                mAfter.foldLeft(fn(v)) {
                  case (acc, (BeforeFieldResult(cv, _, _), mv, m: MiddlewareAfterField[Ctx])) ⇒
                    m.afterField(
                      ...
                      .asInstanceOf[Option[NewVal]] getOrElse acc
                  case (acc, _) ⇒ acc
                }
              ...
              try {
                      ...
                      StandardFieldResolution(
                        ...
                        Some(
                          MappedCtxUpdate(
                            ...
                            if (mError.nonEmpty) doErrorMiddleware
                            else identity
                          )
                        )
                      )
                ...
                case NonFatal(e) ⇒
                  try {
                    ...
                    case NonFatal(me) ⇒
                      ErrorFieldResolution(errors.add(path, e, astField.location).add(path, me, astField.location))
                  }
              }
            } catch {
                ...
                ff.fields.collect {
                  case CollectedField(_, _, Success(fields))
                      if objTpe
                        ...
                        .nonEmpty && !objTpe.getField(schema, fields.head.name).head.tags.contains(ProjectionExclude) ⇒
        ...
        value =
          if (optional && other.value.isEmpty)
            value map (
              v ⇒
                marshaller
                  .addMapNodeElem(v.asInstanceOf[marshaller.MapBuilder], key, marshaller.nullNode, optional = false)
            )
          else
            for { myVal ← value; otherVal ← other.value } yield
              marshaller.addMapNodeElem(
  ...
  case class ParentDeferredContext(uc: Ctx, expectedBranches: Int) {
    val children = Vector.fill(expectedBranches)(ChildDeferredContext(Promise[Vector[Future[Vector[Defer]]]]()))
    ...
    def init(): Unit =
      Future.sequence(children.map(_.promise.future)).onComplete { res ⇒
        ...
        if (allDeferred.nonEmpty)
          resolveDeferredWithGrouping(allDeferred).foreach(groups ⇒ groups.foreach(group ⇒ resolveDeferred(uc, group)))
      }
  }
...
object Resolver {
  ...
  def marshalAstValue(
    ...
    case ast.ObjectValue(values, _, _) ⇒
      marshaller.mapNode(values map (v ⇒ v.name → marshalAstValue(v.value, marshaller, typeName, scalarInfo)))
