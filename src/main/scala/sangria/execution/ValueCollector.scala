...
class ValueCollector[Ctx, Input](
...
)(implicit um: InputUnmarshaller[Input]) {
  ...
  def getVariableValues(
    ...
    if (!um.isMapNode(inputVars))
      ...
      val res = definitions.foldLeft(Vector.empty[(String, Either[Vector[Violation], VariableValue])]) {
        case (acc, varDef) ⇒
          val value = schema
            ...
            .map(
              coercionHelper
                .getVariableValue(varDef, _, um.getRootMapValue(inputVars, varDef.name), fromScalarMiddleware)
            )
            .getOrElse(
              Left(
                Vector(
      ...
      if (errors.nonEmpty)
        Failure(VariableCoercionError(errors.collect { case (name, Left(errors)) ⇒ errors }.flatten, exceptionHandler))
  ...
  def getArgumentValues(
    ...
    ValueCollector.getArgumentValues(
      coercionHelper,
      forAstNode,
      argumentDefs,
      argumentAsts,
        ...
        case (acc, argDef) ⇒
          ...
          try {
            ...
            case InputParsingError(e) ⇒
              errors ++= e.map(
                InvalidInputValueViolation(argDef.name, _, sourceMapper, astValue.flatMap(_.location).toList)
              )
              acc
          }
      }
...
case class VariableValue(
  fn: (ResultMarshaller, ResultMarshaller, InputType[_]) ⇒ Either[Vector[Violation], Trinary[ResultMarshaller#Node]]
) {
  ...
  ): Either[Vector[Violation], Trinary[firstKindMarshaller.Node]] =
    cache
      .getOrElseUpdate(
        System.identityHashCode(firstKindMarshaller) → System.identityHashCode(actualType.namedType),
