...
case class InputDocumentMaterializer[Vars](schema: Schema[_, _], variables: Vars = InputUnmarshaller.emptyMapVars)(
  implicit iu: InputUnmarshaller[Vars]
) {
  def to[T](
  ...
  def inferVariableDefinitions[T](document: InputDocument, inputType: InputType[T]) = {
    document.values.flatMap { v ⇒
      AstVisitor
        .visitAstWithState(schema, v, new mutable.HashMap[String, VariableDefinition]) { (typeInfo, state) ⇒
...
object InputDocumentMaterializer {
  def to[T](schema: Schema[_, _], document: InputDocument, inputType: InputType[T])(
  ...
  def to[T, Vars](schema: Schema[_, _], document: InputDocument, inputType: InputType[T], variables: Vars)(
    implicit iu: InputUnmarshaller[Vars],
  ...
  private def emptyStubSchema[T : FromInput](inputType: InputType[T]): Schema[_, _] =
    Schema(
      ObjectType(
        "Query",
        fields[Unit, Unit](
          Field("stub", StringType, arguments = Argument("stub", inputType) :: Nil, resolve = _ ⇒ "stub")
        )
      )
    )
