...
class ResultResolver(
  ...
  private def createLocation(loc: AstLocation) =
    marshaller.mapNode(
      Seq(
  ...
  private def errorNode(
    ...
    additionalExtensionFields: Seq[(String, marshaller.Node)] = Nil
  ): marshaller.Node =
    mapNode(
  ...
  case class ErrorRegistry(errorList: Vector[marshaller.Node], originalErrors: Vector[RegisteredError]) {
    ...
    def append(path: ExecutionPath, errors: Vector[Throwable], position: Option[AstLocation]) =
      copy(
        ...
        if (preserveOriginalErrors) errors.map(e ⇒ RegisteredError(path, e, position)) ++ originalErrors
        else originalErrors
      )
