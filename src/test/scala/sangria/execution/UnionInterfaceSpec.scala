...
class UnionInterfaceSpec extends WordSpec with Matchers with AwaitSupport {
  ...
  val CatType = ObjectType[Unit, Cat]("Dog", interfaces = NamedType :: Nil, fields = List[Field[Unit, Cat]](
    Field("meows", OptionType(BooleanType), resolve = _.value.meows)))

  val PetType = UnionType[Unit]("Pet", types = DogType :: CatType :: Nil)

  val PersonType = ObjectType[Unit, Person]("Dog", interfaces = NamedType :: Nil, fields = List[Field[Unit, Person]](
  ...
  "Execute: Union and intersection types" should {
    "introspect on union and intersection types" in (pending)
  }
}
