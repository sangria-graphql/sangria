...
* High-level API for SDL-based schema materialization (#288). It provides much more simple and robust API for building an executable schema based on SDL definitions. For more info see ["High-level SDL-based Schema Builder" section of the documentation](http://sangria-graphql.org/learn/#high-level-sdl-based-schema-builder). As a part of this feature, new functionality was introduced in `AstSchemaBuilder` which provides even more flexibility for schema materialization. **Minor breaking change:** most of the methods in `AstSchemaBuilder` got new argument `origin: MatOrigin`. In order to migrate, you need to adjust the signatures of affected methods and add `origin: MatOrigin` argument.
